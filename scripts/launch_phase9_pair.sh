...
ORDER="${STEADYSKY_PHASE9_ORDER:-regional_distribution_rollout,coupled_regional_distribution_rollout}"
...
if [[ "${1:-}" == "--dry-run" ]]; then
  ...
  echo "usage: launch_phase9_pair.sh [--dry-run]" >&2
  exit 2
fi

IFS=',' read -r -a ARMS <<< "${ORDER}"
for ARM in "${ARMS[@]}"; do
  if [[ "${ARM}" != "regional_distribution_rollout" && "${ARM}" != "coupled_regional_distribution_rollout" ]]; then
    echo "Invalid arm in STEADYSKY_PHASE9_ORDER: ${ARM}" >&2
    exit 3
  fi
done
...
mkdir -p "${ROOT}/logs"
MANIFEST="${ROOT}/logs/phase9_pair_launch_$(date -u +%Y%m%dT%H%M%SZ).txt"
{
...
for IDX in "${!ARMS[@]}"; do
  ARM="${ARMS[$IDX]}"
  GPU="${IDX}"
  LOG="${ROOT}/logs/phase9_${ARM}_launcher_$(date -u +%Y%m%dT%H%M%SZ).log"
...
if [[ "${FAILED}" -ne 0 ]]; then
  echo "phase9_pair_failed manifest=${MANIFEST}" >&2
  exit 1
fi

echo "phase9_pair_complete manifest=${MANIFEST}"
