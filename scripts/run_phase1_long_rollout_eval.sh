...
ARM="${1:?usage: run_phase1_long_rollout_eval.sh raw|fourier|mixed|residual|freq_loss|freq_anom|residual_soft|residual_rollout|energy_rollout|spectrum_rollout|attractor_rollout|distribution_rollout|long_distribution_rollout|long_distribution_rollout_best|anchored_distribution_rollout [rollout_months]}"
...
if [[ "${ARM}" != "raw" && "${ARM}" != "fourier" && "${ARM}" != "mixed" && "${ARM}" != "residual" && "${ARM}" != "freq_loss" && "${ARM}" != "freq_anom" && "${ARM}" != "residual_soft" && "${ARM}" != "residual_rollout" && "${ARM}" != "energy_rollout" && "${ARM}" != "spectrum_rollout" && "${ARM}" != "attractor_rollout" && "${ARM}" != "distribution_rollout" && "${ARM}" != "long_distribution_rollout" && "${ARM}" != "long_distribution_rollout_best" && "${ARM}" != "anchored_distribution_rollout" ]]; then
  echo "ARM must be raw, fourier, mixed, residual, freq_loss, freq_anom, residual_soft, residual_rollout, energy_rollout, spectrum_rollout, attractor_rollout, distribution_rollout, long_distribution_rollout, long_distribution_rollout_best, or anchored_distribution_rollout" >&2
  exit 2
fi
