...
ARM="${1:?usage: run_phase1_long_rollout_eval.sh raw|fourier|mixed|residual|freq_loss|freq_anom|residual_soft|residual_rollout|energy_rollout|spectrum_rollout|attractor_rollout|distribution_rollout|long_distribution_rollout|long_distribution_rollout_best|anchored_distribution_rollout|drift_distribution_rollout|spectral_distribution_rollout|regional_distribution_rollout|coupled_regional_distribution_rollout|latent_distribution_rollout|latent_recon_distribution_rollout [rollout_months]}"
...
if [[ "${ARM}" != "raw" && "${ARM}" != "fourier" && "${ARM}" != "mixed" && "${ARM}" != "residual" && "${ARM}" != "freq_loss" && "${ARM}" != "freq_anom" && "${ARM}" != "residual_soft" && "${ARM}" != "residual_rollout" && "${ARM}" != "energy_rollout" && "${ARM}" != "spectrum_rollout" && "${ARM}" != "attractor_rollout" && "${ARM}" != "distribution_rollout" && "${ARM}" != "long_distribution_rollout" && "${ARM}" != "long_distribution_rollout_best" && "${ARM}" != "anchored_distribution_rollout" && "${ARM}" != "drift_distribution_rollout" && "${ARM}" != "spectral_distribution_rollout" && "${ARM}" != "regional_distribution_rollout" && "${ARM}" != "coupled_regional_distribution_rollout" && "${ARM}" != "latent_distribution_rollout" && "${ARM}" != "latent_recon_distribution_rollout" ]]; then
  echo "ARM must be raw, fourier, mixed, residual, freq_loss, freq_anom, residual_soft, residual_rollout, energy_rollout, spectrum_rollout, attractor_rollout, distribution_rollout, long_distribution_rollout, long_distribution_rollout_best, anchored_distribution_rollout, drift_distribution_rollout, spectral_distribution_rollout, regional_distribution_rollout, coupled_regional_distribution_rollout, latent_distribution_rollout, or latent_recon_distribution_rollout" >&2
  exit 2
fi
...
elif [[ "${ARM}" == "long_distribution_rollout" || "${ARM}" == "anchored_distribution_rollout" ]]; then
  RUN_NUM="phase7_${ARM}_edim384"
elif [[ "${ARM}" == "long_distribution_rollout_best" ]]; then
  RUN_NUM="phase7_long_distribution_rollout_best_edim384"
elif [[ "${ARM}" == "drift_distribution_rollout" || "${ARM}" == "spectral_distribution_rollout" ]]; then
  RUN_NUM="phase8_${ARM}_edim384"
elif [[ "${ARM}" == "regional_distribution_rollout" || "${ARM}" == "coupled_regional_distribution_rollout" ]]; then
  RUN_NUM="phase9_${ARM}_edim384"
elif [[ "${ARM}" == "latent_distribution_rollout" || "${ARM}" == "latent_recon_distribution_rollout" ]]; then
  RUN_NUM="phase10_${ARM}_edim384"
else
