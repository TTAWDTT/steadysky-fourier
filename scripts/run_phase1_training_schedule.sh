...
ARM="${1:?usage: run_phase1_training_schedule.sh raw|fourier|mixed|residual|freq_loss|freq_anom|residual_soft|residual_rollout|energy_rollout|spectrum_rollout|attractor_rollout [comma_separated_stage_epochs]}"
...
if [[ "${ARM}" == "residual_rollout" || "${ARM}" == "energy_rollout" || "${ARM}" == "spectrum_rollout" || "${ARM}" == "attractor_rollout" ]]; then
...
if [[ "${ARM}" == "freq_loss" || "${ARM}" == "freq_anom" || "${ARM}" == "energy_rollout" || "${ARM}" == "spectrum_rollout" || "${ARM}" == "attractor_rollout" ]]; then
...
if arm in {"residual_rollout", "energy_rollout", "spectrum_rollout", "attractor_rollout"}:
